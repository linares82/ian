...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('user.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">Id</th>
				<th field="username" sortable="true">Usuario</th>
				<th field="email" sortable="true">Email</th>
				<th field="activated" sortable="true">Activado</th>
				<th field="banned" sortable="true">Prohibido</th>
				<th field="attempts" sortable="true">Intentos</th>
				<th field="suspended" sortable="true">Bloqueado</th>
			</tr>
		</thead>
	</table>
	<div id="toolbar">
		...
		{{ link_to_route('user.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		@endif
		@if (Sentry::getUser()->hasAccess('users-show')) 
		...
		@endif
		@if (Sentry::getUser()->hasAccess('users-bannedReg')) 
		<a href="#" class="easyui-linkbutton" iconCls="icon-forbidden" plain="true" onclick="bannedReg()">Prohibido</a>
		@endif
		@if (Sentry::getUser()->hasAccess('users-unblock')) 
		<a href="#" class="easyui-linkbutton" iconCls="icon-unblock" plain="true" onclick="unblock()">Desbloquear</a>
		@endif
		</div>
		<div>
...
<script type="text/javascript">
	...
	function bannedReg(){
			var row = $('#dg').datagrid('getSelected');
			if (row){
				$.messager.confirm('Confirmar','¿Estas seguro de banear este registro?',function(r){
					if (r){
						$.post('bannedReg/'+row.id,function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
								$.messager.show({	// show error message
	...
	function unblock(){
			var row = $('#dg').datagrid('getSelected');
			if (row){
				$.messager.confirm('Confirmar','¿Estas seguro de desbloquear este registro?',function(r){
					if (r){
						$.post('unblock/'+row.id,function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
								$.messager.show({	// show error message

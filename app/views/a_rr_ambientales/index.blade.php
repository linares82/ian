...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('a_rr_ambientale.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">id</th>
				<th field="material" sortable="true">Material</th>
				<th field="categoria" sortable="true">Categoria</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('a_rr_ambientale-create')) 
		{{ link_to_route('a_rr_ambientale.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<div>
			...
			Material:{{ Form::select('material_id', $materiales_ls, null, array('id'=>'material_id'))  }}
			Categoria:{{ Form::select('categoria_id', $categorias_ls, null, array('id'=>'categoria_id'))  }}
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'a_rr_ambientale.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('a_rr_id', Input::old('a_rr_id'), array('value'=>'0', 'id'=>'a_rr_id')) }}
		        </div>
		        ...
		        <div class="row">
		            {{ Form::hidden('a_st_rr_id', Input::old('a_st_rr_id'), array('value'=>'0', 'id'=>'a_st_rr_id')) }}
		        </div>
...
<script type="text/javascript">
	...
	function stEnProceso(){
		...
		if (row){
			...
			$('#a_rr_id').val(row.id);
			$('#a_st_rr_id').val('2');
			url="{{ route('a_rr_ambientale.comentarioEstatus') }}";
		}else{
	...
	function doSearch(){
	    $('#dg').datagrid('load',{
		    id: $('#idbox').val(),
		    material: $("select#material_id option:selected").val(),
		    categoria: $("select#categoria_id option:selected").val(),

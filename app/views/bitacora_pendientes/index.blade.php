...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('bitacora_pendiente.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">Id</th>
				<th field="pendiente" sortable="true">Pendiente</th>
				<th field="fec_planeada" sortable="true">Fecha</th>
				<th field="estatus" sortable="true">Estatus</th>
				<th field="dias_aviso" sortable="true", hidden="true">dias_aviso</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('bitacora_pendiente-create')) 
		{{ link_to_route('bitacora_pendiente.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<a href="#" class="easyui-menubutton" data-options="menu:'#mm1'">Estatus</a>
			<div id="mm1" style="width:150px;">
				@if (Sentry::getUser()->hasAccess('ln_dossier-comentarioEnProceso')) 
					<div data-options="iconCls:'icon-comentario'"><a href="#" onclick="stEnAtencion()">En atención</a></div>
				@endif	
				@if (Sentry::getUser()->hasAccess('ln_dossier-comentarioTerminado')) 
					<div data-options="iconCls:'icon-atendido'"><a href="#" onclick="stPendiente()">Pendiente</a></div>
				@endif
				@if (Sentry::getUser()->hasAccess('ln_dossier-comentarioInactivo')) 
					<div data-options="iconCls:'icon-finalizado'"><a href="#" onclick="stCerrado()">Cerrado</a></div>
				@endif	
			</div>
		</div>
		<div>
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'a_archivo.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('pendiente_id', Input::old('pendiente_id'), array('value'=>'0', 'id'=>'pendiente_id')) }}
		        </div>
...
<script type="text/javascript">
	var url;
	var row;
	function stEnAtencion(){
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','Cambio Estatus: En Atención');
			$('#estatus_form').form('clear');
			$('#pendiente_id').val(row.id);
			$('#estatus_id').val('2');
			url="{{ route('bitacora_pendiente.comentarioEstatus') }}";
		}else{
			alert("Seleccionar registro valido");
		}
	}
	function stPendiente(){
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','Cambio Estatus: Pendiente');
			$('#estatus_form').form('clear');
			$('#pendiente_id').val(row.id);
			$('#estatus_id').val('3');
			url="{{ route('bitacora_pendiente.comentarioEstatus') }}";
	...
	function stCerrado(){
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','Cambio Estatus: Cerrado');
			$('#estatus_form').form('clear');
			$('#pendiente_id').val(row.id);
			$('#estatus_id').val('4');
			url="{{ route('bitacora_pendiente.comentarioEstatus') }}";

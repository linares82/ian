...
{{ Form::model($m_imp_real, array('class' => 'form', 'method' => 'PUT', 'url' => 'm_imp_real/update/'.$m_imp_real->id)) }}
     <div class="easyui-tabs" style="width:auto;height:auto;">
        ...
        <div class="row_1 @if ( $errors->has('probabilidad_id')) has-error @endif">
            {{ Form::label('probabilidad_id', 'Probabilidad_id:') }}
              {{ Form::input('number', 'probabilidad_id', Input::old('probabilidad_id')) }}
            {{ $errors->first('probabilidad_id', '<div class="errorMessage">:message</div>') }}
        </div>

        <div class="row_1 @if ( $errors->has('imp_real_id')) has-error @endif">
            {{ Form::label('imp_real_id', 'Imp_real_id:') }}
              {{ Form::input('number', 'imp_real_id', Input::old('imp_real_id')) }}
            {{ $errors->first('imp_real_id', '<div class="errorMessage">:message</div>') }}
        </div>
...
<div class="row_buttons">
  ...
  {{ link_to_route('m_imp_real.index', 'Cancelar', $m_imp_real->id, array('class' => 'easyui-linkbutton', 'style'=>'height:30px;width:100px;')) }}
</div>

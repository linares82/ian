...
{{ Form::open(array('route' => 'consulta.consumible', 'class' => 'form', 'method' => 'POST')) }}
    <div class="easyui-tabs" style="width:auto;height:auto;">
        ...
        <div class="row_2 @if ( $errors->has('consumible_f')) has-error @endif">
            {{ Form::label('consumible_f', 'Consumible de:') }}
              {{ Form::select('consumible_f', $consumibles_ls, Input::old('consumible_f'))  }}
            {{ $errors->first('consumible_f', '<div class="errorMessage">:message</div>') }}
        </div>
        <div class="row_2 @if ( $errors->has('consumible_t')) has-error @endif">
            {{ Form::label('consumible_t', 'Consumible a:') }}
              {{ Form::select('consumible_t', $consumibles_ls, Input::old('consumible_t'))  }}
            {{ $errors->first('consumible_t', '<div class="errorMessage">:message</div>') }}
        </div>

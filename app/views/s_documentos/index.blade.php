...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('s_documento.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">id</th>
				<th field="cat_doc" sortable="true">Documento</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('s_documento-create')) 
		{{ link_to_route('s_documento.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<div>
			...
			Documento:{{ Form::select('documento_id', $cat_docs_ls, null, array('id'=>'documento_id'))  }}
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'a_procedimiento.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('s_documento_id', Input::old('s_documento_id'), array('value'=>'0', 'id'=>'s_documento_id')) }}
		        </div>
...
<script type="text/javascript">
	...
	function stEnProceso(){
		...
		if (row){
			...
			$('#s_documento_id').val(row.id);
			$('#estatus_id1').val('2');
			url="{{ route('s_documento.comentarioEstatus') }}";
		}else{

...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('s_procedimiento.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">Id</th>
				<th field="tpo_procedimiento" sortable="true">T. Procedimiento</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('s_procedimiento-create')) 
		{{ link_to_route('s_procedimiento.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<div>
			...
			T. Procedimiento:{{ Form::select('tpo_procedimiento_id', $tpo_procedimientos_ls, null, array('id'=>'tpo_procedimiento_id'))  }}
			T. Documento:{{ Form::select('tpo_documento_id', $tpo_documentos_ls, null, array('id'=>'tpo_documento_id'))  }}
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'a_procedimiento.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('procedimiento_id', Input::old('procedimiento_id'), array('value'=>'0', 'id'=>'procedimiento_id')) }}
		        </div>
...
<script type="text/javascript">
	...
	function stEnProceso(){
		...
		if (row){
			...
			$('#procedimiento_id').val(row.id);
			$('#estatus_id1').val('2');
			url="{{ route('s_procedimiento.comentarioEstatus') }}";
		}else{
	...
	function doSearch(){
	    $('#dg').datagrid('load',{
		    id: $('#idbox').val(),
		    tpo_procedimiento: $("select#tpo_procedimiento_id option:selected").val(),
		    tpo_documento: $("select#tpo_documento_id option:selected").val(),

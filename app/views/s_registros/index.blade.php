...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('s_registro.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true">Id</th>
				<th field="grupo_norma" sortable="true">Grupo</th>
				<th field="norma" sortable="true">Norma</th>
				<th field="elemento" sortable="true">Elemento</th>
				<th field="detalle" sortable="true">Detalle</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('s_registro-create')) 
		{{ link_to_route('s_registro.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<div>
			...
			Grupo:{{ Form::select('grupo_id', $grupos_ls, null, array('id'=>'grupo_id'))  }}
			Norma:{{ Form::select('norma_id', $normas_ls, null, array('id'=>'normas_id'))  }}
			Elemento:{{ Form::select('elemento_id', $elementos_ls, null, array('id'=>'elemento_id'))  }}
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'a_procedimiento.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('registro_id', Input::old('registro_id'), array('value'=>'0', 'id'=>'registro_id')) }}
		        </div>
		        ...
		        <div class="row">
		            {{ Form::hidden('estatus_id', Input::old('estatus_id'), array('value'=>'0', 'id'=>'estatus_id1')) }}
		        </div>
...
<script type="text/javascript">
	...
	function stEnProceso(){
		...
		if (row){
			...
			$('#registro_id').val(row.id);
			$('#estatus_id1').val('2');
			url="{{ route('s_registro.comentarioEstatus') }}";
		}else{
	...
	function doSearch(){
	    $('#dg').datagrid('load',{
		    id: $('#idbox').val(),
		    grupo: $("select#grupo_id option:selected").val(),
		    norma: $("select#norma_id option:selected").val(),
		    elemento: $("select#elemento_id option:selected").val(),

...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('rev_requisitos_ln.contentListIndex', array('rev_requisito'=>$rev_requisito)) }}',autoRowHeight:false,pageSize:50, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="impacto" sortable="true">Impacto</th>
				<th field="condicion" sortable="true" width="100">Condicion</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('rev_requisitos_ln-create')) 
		{{ link_to_route('rev_requisitos_ln.create', 'Crear', array('id'=>$rev_requisito), array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
...
<script type="text/javascript">
    ...
    function cellStyler1(value,row,index){
		...
		var r=f1-f2;
		r=((((r/1000)/60)/60)/24)*-1;
        if (parseInt(row.dias_restantes) > parseInt(row.dias_advertencia1)){
            return 'background-color:#088A08;color:white;';
        }else if(parseInt(row.dias_restantes)<=parseInt(row.dias_advertencia1) &
        		parseInt(row.dias_restantes)>parseInt(row.dias_advertencia2) ){
        	 return 'background-color:#F7FE2E;color:black;';
        }else if(parseInt(row.dias_restantes)<=parseInt(row.dias_advertencia2) &
        		parseInt(row.dias_restantes)>parseInt(row.dias_advertencia3)){
        	return 'background-color:#FE642E;color:white;';
        }else if(parseInt(row.dias_restantes)<=parseInt(row.dias_advertencia3) &
        		parseInt(row.dias_restantes)>0){

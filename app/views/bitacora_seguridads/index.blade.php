...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('bitacora_seguridad.contentListIndex', null)}}',autoRowHeight:false,pageSize:50, 
			pagination:true"
		...
		<thead>
			<tr>
				...
				<th field="inconformidad" sortable="true">No Conformidad</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('bitacora_seguridad-create')) 
		{{ link_to_route('bitacora_seguridad.create', 'Crear', null, array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
	...
	<div id="dlg" class="easyui-dialog" style="width:700px;height:300px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        {{ Form::open(array('route' =>'bitacora_seguridad.comentarioEstatus', 'class' => 'form', 'id'=>'estatus_form')) }}
        	<div class="row">
		        ...
		        <div class="row">
		              {{ Form::hidden('bitacora_seguridad_id', Input::old('bitacora_seguridad_id'), array('value'=>'0', 'id'=>'bitacora_seguridad_id')) }}
		        </div>
...
<script type="text/javascript">
	...
	function stAtendiendo(){
		...
		if (row){
			...
			$('#bitacora_seguridad_id').val(row.id);
			$('#estatus_id').val('2');
			url="{{ route('bitacora_seguridad.comentarioEstatus') }}";
		}else{

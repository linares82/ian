...
@section('contenido_tab')
	...
	<table id="dg" class="easyui-datagrid" style="width:700px;height:200px"
			...
			url:'{{route('checkl.contentListIndex', array('check'=>$check))}}',autoRowHeight:false,pageSize:50, rownumbers:true, 
			pagination:true"
			fit="true" fitColumns="true" >
		<thead>
			<tr>
				<th field="id" sortable="true" hidden="true">Id</th>
				<th field="area" sortable="true">Area de Chequeo</th>
				<th field="norma" sortable="true">Norma</th>
				<th field="no_conformidad" sortable="true">No conformidad</th>
				<th field="requisito" sortable="true">requisito</th>
				<th field="cumplimiento" sortable="true">Cumplimiento</th>
	...
	<div id="toolbar">
		<div>
		@if (Sentry::getUser()->hasAccess('checkl-create')) 
		{{ link_to_route('checkl.create', 'Crear', array('id'=>$check), array('class' => 'easyui-linkbutton', 'iconCls'=>'icon-add')) }}
		@endif
		...
		<div>
			...
			Area: {{ Form::select('area', $areas_ls,Input::old('area_box'), array('id'=>'area_box'))  }}
			Norma: {{ Form::select('norma', $normas_ls,Input::old('norma_box'), array('id'=>'norma_box'))  }}
			Cumplimiento: {{ Form::select('cumplimiento', $cumplimiento_ls,Input::old('cumplimiento_box'), array('id'=>'cumplimiento_box'))  }}
	...
	function doSearch(){
	    $('#dg').datagrid('load',{
		    id: $('#idbox').val(),
		   	area: $('#area_box').val(),
		    norma: $('#norma_box').val(),
		    cumplimiento: $('#cumplimiento_box').val(),
	    });
    }

...
class RevDocumentalVencimiento extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'ian:revDocumentalVencimiento';
				...
				foreach($responsables as $r){
					...
					$registros=DB::Table('rev_documentoal_lns as rdl')
								->select('m.mes', 'rd.anio', 'e.abreviatura','td.tpo_doc','documento',
									'st.estatus', 'i.importancia', 'fec_vencimiento',
									Db::Raw('DATEDIFF(rdl.fec_vencimiento,CURDATE()) as dias_restantes'),
					...
					if($bnd_advertencia1==1){
						Mail::queue('mails.revDocumentalVencimiento', 
							...
							function($message) use($mail, $jefe1_mail, $jefe2_mail, $bnd_advertencia2, $bnd_advertencia3) 
						{
						    ...
						    $message->subject('Siam:Revision Documental Vencimiento');
						});	
					}
				}
